...
public class BeanTransformerAdapter<T> implements ResultTransformer {
    private static Logger logger = LoggerFactory.getLogger(BeanTransformerAdapter.class);
    /**
        ...
        for (int i = 0; i < aliases.length; i++) {
            ...
            if (pd != null) {
                try {
                    ...
                    try {
                        //Oracle的boolean类型会返回BigDecimal，此处特殊处理
                        if (value != null && (boolean.class.equals(pd.getPropertyType()) || Boolean.class.equals(pd.getPropertyType()))) {
                        ...
                        if (value == null && primitivesDefaultedForNullValue) {
                            logger.error("Intercepted TypeMismatchException for column " + column + " and column '"
                                    + column + "' with value null when setting property '" + pd.getName() + "' of type " + pd.getPropertyType()
